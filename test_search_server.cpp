...
void TestFilteringByPredicate() {
    ...
    // status predicate
    {
        const auto& filtered_docs = server.FindTopDocuments(
            "city"s, [](int, DocumentStatus status, int) { return status == DocumentStatus::ACTUAL; });
    ...
    // rating predicate
    {
        const auto& filtered_docs =
            server.FindTopDocuments("city"s, [](int, DocumentStatus, int rating) { return rating == 2; });
    ...
    {
        const auto& filtered_docs =
            server.FindTopDocuments("city"s, [](int document_id, DocumentStatus, int) { return document_id == 1; });
...
void TestSplitIntoWordsEscapesSpaces() {
    ASSERT_EQUAL((std::vector<std::string>{"hello"s, "bro"s}),
                 string_processing::SplitIntoWords("   hello    bro    "s));
