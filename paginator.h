...
class IteratorRange {
   public:
    IteratorRange(InputIterator range_begin, InputIterator range_end)
        : begin_iterator_(range_begin), end_iterator_(range_end) {}

   public:
    InputIterator begin() const { return begin_iterator_; }

    InputIterator end() const { return end_iterator_; }

    size_t size() const { return std::distance(begin_iterator_, end_iterator_); }
...
class Paginator {
   ...
   public:
    bool IsInitialized() { return !(pages_.begin() == pages_.end()); }

    auto begin() const { return pages_.begin(); }

    auto end() const { return pages_.end(); }

    size_t Size() { return pages_.size(); }
